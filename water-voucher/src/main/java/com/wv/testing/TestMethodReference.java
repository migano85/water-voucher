...
//	    Comparator<String> compExample1 = (first, second)->Integer.compare(first.length(), second.length());
	    ...
	    Thread th = new Thread(tmr::nonSTATICsaySomething);//Thread takes as input Runnable functional interface, which we can implement using Lambda OR function reference (::)
